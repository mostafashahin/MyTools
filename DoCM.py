...
def loadTrainingData(sPhone,sDataPath,iNumAttributes):
    #print('loading',sPhone,sDataPath,iNumAttributes)
...
def EvaluatePhone(sMlfFile, sPhoneAttrFile,sFileList,th=0.25,phTh=''):
    with open(sFileList) as fList:
        lFiles = [splitext(basename(sFile))[0] for sFile in fList.read().splitlines()]
    if phTh != '':
        with open(phTh) as fP:
            dPhTh = dict([tuple(sLine.split()) for sLine in fP.read().splitlines()])
    dPhonAttr = {}
    ...
    for i in range(0,len(lSEInx),2):
        ...
        for j in range(iSinx+1,iEinx):
            ...
            if phTh != '':
                th = float(dPhTh[sPhoneme])
            if iNumInClassFrams >= th*iNumOutClassFrams:
                _class = 1
            else:
                _class = 0
            print(sPhoneme,iNumInClassFrams,iNumOutClassFrams,sFileName,th)
...
def ComputeAcc(dFileEval,sRefMLF):
    ...
    for i in range(0,len(lSEInx),2):
        ...
        if sFileName not in dFileEval:
            continue
        dFiles[sFileName] = [0,0,0,0]
        lEval = dFileEval[sFileName]
        iIndx = 0
        for line in lMlf[iSinx+1:iEinx]:
            sPhone = line.split('-')[-1]
            sPhone = sPhone.replace('*','').replace('+','').replace('0','').replace('1','').replace('2','')
            if sPhone == 'sil':
                #iIndx += 1
                continue
            ...
            if '-' in line or '*' in line:
                if lEval[iIndx][1] == 0:
                    dPhone[sPhone][1] += 1
                    dFiles[sFileName][1] += 1
                    iCR += 1
                    fTotal += 1
                else:
                    dPhone[sPhone][2] += 1
                    dFiles[sFileName][2] += 1
                    iFA += 1
                    fTotal += 1
            elif '+' in line:
                if lEval[iIndx][1] == 0:
                    ...
                    dPhone[sPhone][0] += 1
                    dFiles[sFileName][0] += 1
                    iCA += 1
                    fTotal += 1
            else:
                if lEval[iIndx][1] == 0:
                    dPhone[sPhone][3] += 1
                    dFiles[sFileName][3] += 1
                    iFR += 1
                    fTotal += 1
                else:
                    dPhone[sPhone][0] += 1
    ...
    P_CA = (float(iCA)/(iCA+iFA+0.000000001)) * 100.0
    P_CR = (float(iCR)/(iCR+iFR+0.000000001)) * 100.0
    R_CA = (float(iCA)/(iCA+iFR+0.000000001)) * 100.0
    R_CR = (float(iCR)/(iCR+iFA+0.000000001)) * 100.0
    FPR = (float(iFA)/(iFA+iCR+0.000000001)) * 100.0
    FNR = (float(iFR)/(iFR+iCA+0.000000001)) * 100.0
    FA = 2*(P_CA*R_CA)/(P_CA+R_CA+0.000000001)
    #print(dPhone)
    return(SA,P_CA,P_CR,R_CA,R_CR,FA,FPR,FNR,dFiles,dPhone)
    #return(dFileEval)

def ComputeAcc_Gen(dFileEval,sRefMLF,lFocusGen=[],lInsertion=[],lExcludedPhones=[]):
    with open(sRefMLF) as fMlf:
        lMlf = fMlf.read().splitlines()
    lSEInx = [i for i in range(len(lMlf)) if lMlf[i][0]=='"' or lMlf[i][0] == '.'] #Get Start and End index of each file
    fTotal = 0.0
    ...
    dPhone = {}
    dFiles = {}
    if not lFocusGen: lFocusGen = range(100)
    stFocusGen = set(lFocusGen)
    for i in range(0,len(lSEInx),2):
        #print(int((float(i)/len(lSEInx))*100),'%',end='\r')
        iSinx = lSEInx[i]
        iEinx = lSEInx[i+1]
        ...
        iIndx = 0
        for line in lMlf[iSinx+1:iEinx]:
            sPhoneMask = line.split()
            sPhone, sMask = sPhoneMask[0], sPhoneMask[1:]
            lExpected = [i for i,x in enumerate(sMask) if x=='1']
            lActual = [i for i,x in enumerate(sMask) if x=='2']
            lIntersection = list(stFocusGen.intersection(lExpected+lActual))
            if not lIntersection or sPhone in lExcludedPhones:
                print(sFileName,sPhone,'NA',iIndx)
                iIndx += 1
                continue
            if sPhone not in dPhone:
                dPhone[sPhone] = [0,0,0,0]
            ...
            if not lActual:
                ...
                if lEval[iIndx][1] == 0:
                    ...
                    Eval = 'FA'
                    fTotal += 1
            print(sFileName,sPhone,Eval,lEval[iIndx][1],iIndx,sMask,lExpected,lActual,lIntersection)
            iIndx += 1
    print(iCA,iCR,iFA,iFR)
    SA = ((iCA+iCR)/fTotal) * 100.0
    P_CA = (float(iCA)/(iCA+iFA+0.000000001)) * 100.0
