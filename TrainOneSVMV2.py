    ...
    for kernel in ['rbf']:#,'sigmoid']:#['poly', 'rbf', 'sigmoid']:
        ...
        #cls    f.fit(arCVData,arCVLab)
                ...
                for sPhone in dPhoneIndx:
                    ...
                    fLog.write(sPhone+'\t'+str(fscoreTemp)+'\t'+str(x)+'\n')
                    fscoreOut += fscoreTemp
                fscore = fscoreIn + fscoreOut
                fLog.write(param+'\t'+str(fCurAcc)+'\t'+str(fscoreIn)+'\t'+str(xIn)+'\t'+str(fscoreOut)+'\t'+str(fscore)+'\n')
                fLog.flush()
                fscore = fCurAcc
                print(param,fCurAcc,fscore)
                if fscore > fMaxAcc:
                    fMaxAcc = fscore
                    sMAXparam = param
